...
void AsmGenerator::generateAssembly(std::shared_ptr<AstNode> ast) {
	...
	switch(ast->type) {
		...
		case funcTerm:
			...
			return;
			break;
		case parameterFunctionPrototype:
		...
		case returnStatement:
			...
			emitln("ret");
			break;
		case identifierAssign:
			offset = getOffset(ast->value);
		...
		case identifierAccess:
			...
			break;
		case numberDeclarations:
			bytes = 4 * std::stoi(ast->value);
